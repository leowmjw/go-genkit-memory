...
// testState is a representative session state type used in tests.
...
func newTestStore(t *testing.T) (*bboltstore.Store[testState], string) {
	t.Helper()
	f, err := os.CreateTemp(t.TempDir(), "bbolt-*.db")
	if err != nil {
	...
	store, err := bboltstore.NewStore[testState](context.Background(), path)
	if err != nil {
...
// TestNewStore_CreatesFile verifies that NewStore creates the database file.
...
// TestNewStore_CustomBucket verifies that a custom bucket name is accepted.
func TestNewStore_CustomBucket(t *testing.T) {
	dir := t.TempDir()
	path := dir + "/test.db"
	store, err := bboltstore.NewStore[testState](context.Background(), path,
		bboltstore.WithBucket("custom-bucket"),
	)
	if err != nil {
		t.Fatalf("NewStore with custom bucket failed: %v", err)
	}
	defer store.Close()
}

// TestNewStore_EmptyBucketReturnsError verifies that an empty bucket name fails.
func TestNewStore_EmptyBucketReturnsError(t *testing.T) {
	dir := t.TempDir()
	_, err := bboltstore.NewStore[testState](context.Background(), dir+"/test.db",
		bboltstore.WithBucket(""),
	)
	if err == nil {
...
func TestPersistence(t *testing.T) {
	...
	{
		store, err := bboltstore.NewStore[testState](ctx, path)
		if err != nil {
...
func TestConcurrentAccess(t *testing.T) {
	...
	// Final read should return non-nil and valid data.
...
// the BBolt store, mirroring how application code would use it.
func TestSessionIntegration(t *testing.T) {
	store, _ := newTestStore(t)
	ctx := context.Background()

	// Create a new session with the BBolt store.
	...
	// Update state several times (simulating multi-turn conversation).
	for i := 1; i <= 5; i++ {
		state := sess.State()
		state.Count = i
		state.Name = "Turn"
	...
	// Load a fresh Session from the store to simulate a new process.
...
// a NotFoundError (the error type defined in the session package).
